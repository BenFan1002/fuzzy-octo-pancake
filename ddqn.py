...
class DQNAgent:
    ...
    def load_model(self, name):
        try:
            ...
            print(f"Failed to load model weights from {name}: {e}")

    # Save the model which is under training
    def save_model(self, name, episode=None):
        # Add episode to the file name if provided
        filename = f"{name}_episode_{episode}.weights.h5" if episode is not None else f"{name}.weights.h5"
        ...
        for e in range(EPISODES):
            ...
            while not done:
                ...
                if done:
                    ...
                    if agent.train:
                        agent.save_model(args.model + ".weights.h5")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sim",
        type=str,
        default="C:\\Users\\10944\\Code\\CSCE642-Project\\DonkeySimWin\\donkey_sim.exe",
        help="path to unity simulator. maybe be left at manual if you would like to start the sim on your own.",
    )
    parser.add_argument("--model", type=str, default="rl_driver.h5.weights.h5", help="path to model")
