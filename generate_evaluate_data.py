...
# Evaluate Baseline Agent in Faulty Environment
print("Evaluating Baseline Agent in Faulty Environment...")
env_faulty = DonkeyCarFaultyEnv(conf)
baseline_results_faulty = evaluate_agent(env_faulty, baseline_model, num_episodes=num_episodes, is_monitor_model=False)
baseline_results_faulty.to_csv('evaluation_baseline_faulty.csv', index=False)
env_faulty.close()

# Evaluate Faulty Agent without Monitoring in Faulty Environment
print("Evaluating Faulty Agent without Monitoring in Faulty Environment...")
env_faulty = DonkeyCarFaultyEnv(conf)
faulty_results_faulty = evaluate_agent(env_faulty, faulty_model, num_episodes=num_episodes, is_monitor_model=False)
faulty_results_faulty.to_csv('evaluation_faulty_faulty.csv', index=False)
env_faulty.close()

# Evaluate Faulty Agent without Monitoring in Normal Environment (Optional)
print("Evaluating Faulty Agent without Monitoring in Normal Environment...")
env_normal = DonkeyCarNormalEnv(conf)
faulty_results_normal = evaluate_agent(env_normal, faulty_model, num_episodes=num_episodes, is_monitor_model=False)
faulty_results_normal.to_csv('evaluation_faulty_normal.csv', index=False)
env_normal.close()
...
# Evaluate Agent with Monitoring in Normal Environment (Optional)
print("Evaluating Agent with Monitoring in Normal Environment...")


# For this, we need to adjust the environment to not simulate faults
# Create a modified DonkeyCarMMDPEnv that doesn't simulate faults

class DonkeyCarMMDPEnvNoFaults(DonkeyCarMMDPEnv):
    def step(self, action):
        # Same as DonkeyCarMMDPEnv but without simulating faulty sensors
        steering = action[0]
        throttle = action[1]
        monitor_action = action[2]
        ...
        # Do not simulate faulty sensors
        # obs remains as is regardless of monitor state
...
print("Evaluating Agent with Monitoring in Normal Environment...")
env_monitor_normal = DonkeyCarMMDPEnvNoFaults(conf)
monitor_results_normal = evaluate_agent(env_monitor_normal, monitor_model, num_episodes=num_episodes,
                                        is_monitor_model=True)
monitor_results_normal.to_csv('evaluation_monitor_normal.csv', index=False)
env_monitor_normal.close()
